...
export const VirtualizedGrid: React.FC<GridProps> = ({
  products, edits, errors, onCellChange, onSaveRow, savingRows
}) => {
  ...
  const endIndex = Math.min(products.length - 1, Math.floor((scrollTop + viewportHeight) / rowHeight) + 2);

  const visibleProducts = products.slice(startIndex, endIndex + 1);
  const offsetY = startIndex * rowHeight;

  return (
    <div ref={containerRef} style={{ height: viewportHeight, overflowY: 'auto', position: 'relative', border: '1px solid #ccc' }}>
      <div style={{ height: totalHeight, width: '100%', position: 'relative' }}>
        <div style={{ transform: `translateY(${offsetY}px)`, position: 'absolute', left: 0, right: 0 }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', background: '#fff' }}>
            <thead>
              <tr style={{ background: '#f5f5f5', borderBottom: '2px solid #ddd', height: '40px' }}>
                <th>Actions</th><th>Title</th><th>Price</th><th>Stock</th><th>Rating</th><th>Category</th>
              </tr>
            </thead>
            <tbody>
              {visibleProducts.map((product) => {
                ...
                const renderCell = (field: keyof EditState, type = "text") => {
                  const val = rowEdits[field] !== undefined ? rowEdits[field] : product[field];
                  const err = rowErrors[field];
                  return (
                    <td style={{ padding: '8px', borderBottom: '1px solid #ddd', position: 'relative' }}>
                      <input
                        type={type}
                        value={val}
                        style={{ border: err ? '1px solid red' : '1px solid #ccc', width: '90%', padding: '4px' }}
                        onChange={(e) => {
                      ...
                      />
                      {err && <div style={{ color: 'red', fontSize: '10px', position: 'absolute', bottom: -2 }}>{err}</div>}
                    </td>
                  );
                };

                return (
                  <tr key={product.id} style={{ height: rowHeight, background: isDirty ? '#fffde7' : 'transparent' }}>
                    <td style={{ padding: '8px', borderBottom: '1px solid #ddd' }}>
                      <button 
                        disabled={!isDirty || hasError || savingRows[product.id]} 
