...
export const VirtualizedGrid: React.FC<GridProps> = ({
  ...
  const renderCell = (product: Product, field: ColumnKey, type = "text") => {
    ...
    if (field === "title") {
      return (
        <div className={`px-3 py-1.5 relative ${COL_WIDTHS[field]}`}>
          <div className="relative flex items-center">
            <input
              type="text"
              value={val ?? ""}
              className={`w-full rounded-lg border px-2.5 py-1 text-sm shadow-sm transition-all duration-200 focus:outline-none focus:ring-4 ${
                err
            ...
            />
            {err && (
              <div className="absolute top-[38px] left-3 z-45 rounded bg-red-500 px-1.5 py-0.5 text-[9px] font-semibold text-white shadow-md dark:bg-red-600 animate-slide-in pointer-events-none whitespace-nowrap">
                {err}
              </div>
            )}
          </div>
      ...
      <div className={`px-3 py-1.5 relative ${COL_WIDTHS[field]}`}>
        <div className="relative flex items-center">
          <input
            type={type}
            value={val ?? ""}
            className={`w-full rounded-lg border pl-2.5 ${
              field === "discountPercentage" ? "pr-6" : "pr-2.5"
            } py-1 text-sm shadow-sm transition-all duration-200 focus:outline-none focus:ring-4 ${
              err
                ? "border-red-500 focus:border-red-500 focus:ring-red-500/20 dark:border-red-500/80"
                : "border-gray-200 hover:border-gray-300 focus:border-blue-500 focus:ring-blue-500/15 dark:border-slate-600 dark:hover:border-slate-500 dark:focus:border-blue-500"
            } dark:bg-slate-800 dark:text-white dark:placeholder:text-slate-400`}
  ...
  const minWidth = getTableMinWidth(visibleColumns);
    ...
    <div className="rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden dark:border-slate-700 dark:bg-slate-800">
      <div className="overflow-x-auto w-full">
        <div className="flex flex-col" style={{ minWidth }}>
          {/* Header */}
          <div className="bg-gray-50/95 border-b border-gray-200 dark:bg-slate-800/95 dark:border-slate-700">
            <div className="flex items-center" style={{ height: "44px" }}>
              ...
              {FIELD_KEYS.map((key, idx) => {
                ...
                return (
                  <div
                    key={key}
                    className={`px-3 py-2.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider ${COL_WIDTHS[key]} dark:text-slate-400`}
                  >
                    {HEADERS[idx]}
                  </div>
                );
                ...
                {visibleProducts.map((product, idx) => {
                  ...
                  return (
                    <div
                      ...
                      } ${isEditing ? "ring-2 ring-blue-500/20 relative z-20" : ""}`}
                      style={{ height: rowHeight, minHeight: rowHeight, animationDelay: `${idx * 15}ms` }}
                    >
                      <div className="w-11 flex-shrink-0 flex items-center justify-center">
                        <button
